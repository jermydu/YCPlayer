...
namespace YCLIB {
	YCAVReader::YCAVReader() :imp(new Imp) {
	}
	...
	YCRet YCAVReader::Open(const char* path) {
		if (imp->pFormatPrivate->pAvFormatContext == nullptr) {
		...
		int ret = avformat_open_input(&imp->pFormatPrivate->pAvFormatContext, path, nullptr, nullptr);
		if (ret != 0) {
			LOG_ERROR("avformat_open_input failed: error={0}", { GetFfmpegErrorString(ret) });
			return YCRet::YCRet_OpenFileFailed;
		}
		ret = avformat_find_stream_info(imp->pFormatPrivate->pAvFormatContext, nullptr);
		if (ret != 0) {
			...
			//路径名/文件名
			LOG_INFO("media name-->[{0}]", { imp->pFormatPrivate->pAvFormatContext->url });
			//流媒体数量
			LOG_INFO("stream number-->[{0}]", { std::to_string(imp->pFormatPrivate->pAvFormatContext->nb_streams) });
			//媒体文件码率 单位bps
			LOG_INFO("media bitrate-->[{0}kbps]", { std::to_string(imp->pFormatPrivate->pAvFormatContext->bit_rate / 1024) });
			//duration  微妙->秒
			int totalSeconds = imp->pFormatPrivate->pAvFormatContext->duration / AV_TIME_BASE;
			...
			int nAudioIndex = -1;
			int nVideoIndex = -1;
			for (uint32_t i = 0; i < imp->pFormatPrivate->pAvFormatContext->nb_streams; ++i) {
				//获取一个流
				AVStream* pAVStream = imp->pFormatPrivate->pAvFormatContext->streams[i];
				//音频流
				if (AVMEDIA_TYPE_AUDIO == pAVStream->codecpar->codec_type) {
					...
					if (pAVStream->duration != AV_NOPTS_VALUE) {
						//ffmpeg中的内部计时单位（时间基），ffmepg中的所有时间都是基于它为一个单位，
	...
	int YCAVReader::GetStreamCount() const {
		return imp->pFormatPrivate->pAvFormatContext ? imp->pFormatPrivate->pAvFormatContext->nb_streams : 0;
	}
	...
	YCRet YCAVReader::Close() {
		...
		avformat_close_input(&imp->pFormatPrivate->pAvFormatContext);
		return YCRet::YCRet_OK;
	}

	YCRet YCAVReader::Read(YCAVPacket* pYcAvPacket) {
		if (imp->pFormatPrivate->pAvFormatContext == nullptr) {
			return YCRet::YCRet_CreateContextFailed;
		}
		int ret = av_read_frame(imp->pFormatPrivate->pAvFormatContext, pYcAvPacket->imp->pPacketPrivate->pAvPacket);
		if (ret == 0) {
			return YCRet::YCRet_OK;
		} else {
			if (ret == AVERROR_EOF) {
	...
	int YCAVReader::GetVideoStreamIndex()const {
		return av_find_best_stream(imp->pFormatPrivate->pAvFormatContext, AVMediaType::AVMEDIA_TYPE_VIDEO, -1, -1, nullptr, 0);
	}
	int YCAVReader::GetAudioStreamIndex()const {
		return av_find_best_stream(imp->pFormatPrivate->pAvFormatContext, AVMediaType::AVMEDIA_TYPE_AUDIO, -1, -1, nullptr, 0);
